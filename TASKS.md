...
- **Task 8 — design system. DONE 2026-08-22 — "Mark 42".** Four directions were mocked up and
  Mark chose Cinematic: black, one gold accent, thin type at large sizes, space instead of glow.
  The brief was "feel like I'm in the Iron Man helmet", which changed the structure, not just the
  palette.

  Three earlier attempts missed because they all made the same thing — a dark web page with a
  header, cards and a nav bar. What actually changed:
  - **The HUD moved to the periphery.** Inside a helmet the centre of your view is clear, because
    you look through it; the instruments live at the edge. So the frame is fixed to the *viewport*
    — corner brackets at the four corners of vision, status pinned to the edges, a vignette you
    look through — and the content scrolls behind it. `src/ui/helmet.jsx`.
  - **Parallax.** The frame sits on a nearer plane than the content and both drift with the
    pointer. That's what sells a projection floating in front of you rather than ink on a page.
  - **Power-on sequence.** The helmet boots once per session, then gets out of the way.
  - **Navigation is a rail at the edge of vision**, not a bar through the middle of it. It becomes
    a bottom strip under 860px.
  - **No cards.** A panel is a hairline rule and the space beneath it. Boxing everything is what
    made the previous passes read as a web page.
  - Palette: `#060606`, one gold `#C9A227`, warm ivory. Status colours desaturated so nothing
